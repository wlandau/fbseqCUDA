---
title: "`heterosisCUDA` package installation guide"
...
vignette: >
  ...
  \VignetteIndexEntry{`heterosisCUDA` package installation guide}
  \usepackage[utf8]{inputenc}
---

# Purpose

`heterosisCUDA` is the internal engine of the [`heterosis` package](https://github.com/wlandau/heterosis), and it is implemented in [CUDA](http://www.nvidia.com/object/cuda_home_new.html) to provide necessary acceleration for the underlying Markov chain Monte Carlo (MCMC) procedure. `heterosis` and `heterosisCUDA` are kept separate for convenience. For example, you can set up input with `heterosis` and without `heterosisCUDA` on a low-end laptop, run the MCMC remotely with both `heterosis` and `heterosisCUDA` on a CUDA-enabled [cloud computing enterprise](http://www.nvidia.com/object/gpu-cloud-computing-services.html) such as [Amazon Web Services](http://aws.amazon.com/ec2/instance-types/), and analyze the output locally with `heterosis` and without `heterosisCUDA`. `heterosisCUDA` is not meant to be directly accessed by the user, so there are no usage instructions. However, it is important to understand the system requirements and installation instructions below.

# System requirements

- R $\ge$ 3.2.0
- R packages `methods` and `knitr`, available through the [Comprehensive R Archive Network (CRAN](https://cran.r-project.org/).
- R package `heterosis`, available through GitHub. (See the `heterosis` package installation vignette).
...
git clone git@github.com:wlandau/heterosisCUDA.git

```

In your current working directory (check with `pwd`), open  `heterosisCUDA/src/Makevars` in a text editor. The top line should read 
...
```
R CMD build heterosisCUDA
R CMD INSTALL heterosisCUDA_0.0.tar.gz
```

You may have to replace `heterosisCUDA_0.0.tar.gz` by the name of whatever tarball comes out of `R CMD build`.
