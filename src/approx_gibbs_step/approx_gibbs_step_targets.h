#ifndef APPROX_GIBBS_STEP_TARGETS_H
#define APPROX_GIBBS_STEP_TARGETS_H
...
__device__ double ltarget_basic(chain_t *dd, approx_gibbs_args_t args, double x){
...
__device__ double ltarget_gamma(chain_t *dd, approx_gibbs_args_t args, double x){
  if(x < 0.0 || x > args.upperbound) return -CUDART_INF;
  double a = args.shape, b = args.rate;
  return /* a*log(b) - lgamma(a) + */ (a-1)*log(x) - b*x;
}

__device__ double ltarget_inv_gamma(chain_t *dd, approx_gibbs_args_t args, double x){
  if(x < 0.0 || x > args.upperbound) return -CUDART_INF;
  double a = args.shape, b = args.scale;
  return /* a*log(b) - lgamma(a) */ -(a+1)*log(x) - b/x;
}

__device__ double ltarget_nu(chain_t *dd, approx_gibbs_args_t args, double x){
  double ret = -CUDART_INF;
  if(x > 0.0 && x < args.D)
...
__device__ double ltarget_xi_laplace(chain_t *dd, approx_gibbs_args_t args, double x){
  double ret = -CUDART_INF;
  if(x > 0.0)
...
__device__ double ltarget_xi_t(chain_t *dd, approx_gibbs_args_t args, double x){
  double ret = -CUDART_INF;
  if(x > 0.0)
...
__device__ double ltarget_xi_horseshoe(chain_t *dd, approx_gibbs_args_t args, double x){
  double ret = -CUDART_INF;
  if(x > 0.0)
...
__device__ double ltarget(chain_t *dd, approx_gibbs_args_t args, double x){
...
#endif // APPROX_GIBBS_STEP_TARGETS_H
