...
void hh2hd(SEXP hh, chain_t *hd){
  ...
  CUDA_CALL(cudaMemcpy(hd->p, li(hh, "p"), L * sizeof(int), cudaMemcpyHostToDevice));
  ...
  CUDA_CALL(cudaMemcpy(hd->delta, li(hh, "deltaStart"), L * G * sizeof(int), cudaMemcpyHostToDevice));
  ...
  CUDA_CALL(cudaMemcpy(hd->pi, lr(hh, "piStart"), L * sizeof(double), cudaMemcpyHostToDevice));
